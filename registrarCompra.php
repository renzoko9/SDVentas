<?php
    ...
    if(!empty($_REQUEST)){
        session_start();
        if(empty($_REQUEST['id_prov'])){
            $alert='<div class="alert alert-danger" role="alert">Por favor, complete todos los campos requeridos</div>';
        }else{
            ...
            for($i=1; $i<=$contador; $i++){
                ...
                if (mysqli_query($conn, $sql)){
                    $query= mysqli_query($conn, "SELECT * FROM inventario WHERE id_producto = $v2");
                    $cantidadAct = mysqli_fetch_array($query)['cantidad'];
                    $update = ("UPDATE inventario SET cantidad='".($cantidadAct + $v3)."' WHERE id_producto='".$v2."' ");
                            ...
                            <div class="card-body card-block">
                                <form action="" method="post" class="form-horizontal">
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div  class="col-sm-6">
                                            <select class="form-control" name="id_prov">
                                                ...
                                                <?php
                                                include "conexion.php";
                                                $resultadoP = mysqli_query($conn,"SELECT * FROM proveedor ORDER BY nombre");
                                                if(mysqli_num_rows($resultadoP)>0){
                                                    while($dataP=mysqli_fetch_array($resultadoP)){
                                                        $idProv = $dataP['id_proveedor'];
                                                        $nombreProv = $dataP['nombre'];
                                                        $rucProv = $dataP['ruc'];
                                                        ?>
                                                        <option value="<?php echo $idProv; ?>"><?php echo $nombreProv." - RUC ".$rucProv; ?></option>
                                                        <?php
                                                    }
                                                }
                                                ?>
                                    ...
                                    <div class="row form-group">
                                        ...
                                        <div class="col-2 col-md-6 offset-md-3">    
                                            <div style="text-align: center"><button type="submit" class="btn btn-success" id="guardarCambios"><i class="fa ti-check"></i>&nbsp;&nbsp;Registrar Compra</button></div>
                                        </div> 
                                    </div>
                                </form>
